...
static const int CAMERA_PWDN_ASSERT_MS = 3000;
static const int CAMERA_PWDN_RELEASE_MS = 1000;
static const int CAMERA_XCLK_STABLE_MS = 10;
static camera_state_t *s_state = NULL;

#if CONFIG_IDF_TARGET_ESP32S3 // LCD_CAM module of ESP32-S3 will generate xclk
#define CAMERA_ENABLE_OUT_CLOCK(v) ESP_OK
...
static esp_err_t camera_probe(const camera_config_t *config, camera_model_t *out_camera_model)
{
    ...
    *out_camera_model = CAMERA_NONE;
    ...
    if (config->pin_xclk >= 0) {
        ...
        ret = CAMERA_ENABLE_OUT_CLOCK(config);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "xclk init err");
            goto err;
        }
        ESP_LOGI(TAG, "Waiting %d ms for XCLK to stabilize before SCCB/PWDN sequence", CAMERA_XCLK_STABLE_MS);
        vTaskDelay(pdMS_TO_TICKS(CAMERA_XCLK_STABLE_MS));
    }

    if (config->pin_sccb_sda != -1) {
        ESP_LOGI(TAG, "Initializing SCCB");
    ...
    if (config->pin_pwdn >= 0) {
        ...
        ESP_LOGI(TAG, "Camera PWDN asserted for %d ms", CAMERA_PWDN_ASSERT_MS);
        vTaskDelay(pdMS_TO_TICKS(CAMERA_PWDN_ASSERT_MS));
        gpio_set_level(config->pin_pwdn, 0);
        ESP_LOGI(TAG, "Camera PWDN released; waiting %d ms before SCCB probe", CAMERA_PWDN_RELEASE_MS);
        vTaskDelay(pdMS_TO_TICKS(CAMERA_PWDN_RELEASE_MS));
    }
