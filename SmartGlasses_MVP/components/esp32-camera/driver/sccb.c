...
#define SCCB_PROBE_TIMEOUT_MS   100
#define SCCB_PROBE_SETTLE_MS    1
#define OV2640_SCCB_ADDR        0x30
#define OV3660_SCCB_ADDR        0x3C
#define OV3660_SCCB_WRITE_ID    0x78
#define OV3660_SCCB_READ_ID     0x79
...
int SCCB_Init(int pin_sda, int pin_scl)
{
    ...
    if ((ret =  i2c_param_config(sccb_i2c_port, &conf)) != ESP_OK) {
        ESP_LOGE(TAG, "i2c_param_config failed: port=%d sda=%d scl=%d freq=%d ret=0x%x", sccb_i2c_port, pin_sda, pin_scl, SCCB_FREQ, ret);
        return ret;
    }

    ret = i2c_driver_install(sccb_i2c_port, conf.mode, 0, 0, 0);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "i2c_driver_install failed: port=%d ret=0x%x", sccb_i2c_port, ret);
    }
    return ret;
}
...
uint8_t SCCB_Probe(void)
{
    ...
    ESP_LOGI(TAG, "SCCB_Probe: scanning I2C addresses on port %d, freq=%d Hz, timeout=%d ms...", sccb_i2c_port, SCCB_FREQ, SCCB_PROBE_TIMEOUT_MS);
    ...
    for (uint8_t addr = 1; addr < 0x7f; addr++) {
        ...
        i2c_master_stop(cmd);
        esp_err_t ret = i2c_master_cmd_begin(sccb_i2c_port, cmd, pdMS_TO_TICKS(SCCB_PROBE_TIMEOUT_MS));
        i2c_cmd_link_delete(cmd);

        if (ret == ESP_OK) {
            ESP_LOGI(TAG, "SCCB_Probe: found device at addr=0x%02x", addr);
            return addr;
        }

        if (addr == OV3660_SCCB_ADDR || addr == OV2640_SCCB_ADDR || addr == OV7725_SCCB_ADDR) {
            if (addr == OV3660_SCCB_ADDR) {
                ESP_LOGW(TAG, "SCCB_Probe: OV3660 datasheet addr 8-bit write/read=0x%02x/0x%02x, 7-bit scan addr=0x%02x no ACK, ret=0x%x",
                         OV3660_SCCB_WRITE_ID, OV3660_SCCB_READ_ID, addr, ret);
            } else {
                ESP_LOGW(TAG, "SCCB_Probe: expected camera addr=0x%02x no ACK, ret=0x%x", addr, ret);
            }
        }

        if (addr != 0x7e) {
            vTaskDelay(pdMS_TO_TICKS(SCCB_PROBE_SETTLE_MS));
        }
    }

    ESP_LOGE(TAG, "SCCB_Probe: NO device found on any address! Check ribbon cable, module seating, GPIO32 PWDN, GPIO0 XCLK, SDA26/SCL27, and power supply.");
    return slave_addr;
}
